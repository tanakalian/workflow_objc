#include "ObjCMessageHandler.h"
#include <lowlevelilinstruction.h>

using namespace BinaryNinja;

ObjCMessageHandler::ObjCMessageHandler(Ref<BinaryView> data)
{
    if (auto sect = data->GetSectionByName("__objc_stubs"))
    {
        m_hasObjcStubs = true;
        m_objcStubsRange.first = sect->GetStart();
        m_objcStubsRange.second = sect->GetStart() + sect->GetLength();
    }
    else
        m_hasObjcStubs = false;
    
    m_msgSendFunctions = FindMsgSendFunctions(data);
}


std::set<uint64_t> ObjCMessageHandler::FindMsgSendFunctions(BinaryNinja::Ref<BinaryNinja::BinaryView> data)
{
    ...
    for (const auto& c : candidates) {
        if ((authStubsSection && sectionContains(authStubsSection, c))
            ...
            if (!(m_hasObjcStubs && c->GetAddress() > m_objcStubsRange.first && c->GetAddress() < m_objcStubsRange.second))
